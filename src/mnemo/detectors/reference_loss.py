...
that wasn't. We score each candidate by `suspect_logprob - ref_logprob`:
the suspect's edge over a small reference LM (Phi-1.5, Tinystories,
SILO) which was trained on disjoint data.
...
class ReferenceLoss(Detector):
    """Score = mean_logprob(suspect) - mean_logprob(reference).

    Higher ⇒ suspect is much more confident than the reference ⇒ sample
    ...
    def score_sample(
        ...
        if suspect_lp.size == 0 or ref_lp.size == 0:
            return 0.0
        return float(np.mean(suspect_lp) - np.mean(ref_lp))
